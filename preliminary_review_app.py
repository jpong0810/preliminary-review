...
.header-grid, .row-grid { display: grid; grid-template-columns: 3fr 1.4fr repeat(6, 1.05fr) 0.9fr; gap: 10px; align-items: center; }
.header-grid { background: #eef2ff; border: 1px solid var(--line); border-radius: 12px; padding: 10px 12px; font-weight: 700; }
.add-row-grid { display: grid; grid-template-columns: 2fr 1.3fr 0.9fr; gap: 10px; align-items: center; }
.add-btn > button { background: #111827; border: 1px solid #0b1220; color: #fff; font-weight: 700; border-radius: 10px; padding: 8px 14px; }
.html-pill { display: block; width: 100%; border-radius: 10px; padding: 8px; font-weight: 700; border: 2px solid var(--blue); background: var(--blue); color: #ffffff; text-align: center; cursor: pointer; }
.html-pill.done { background: #ffffff; color: var(--blue); }
.html-pill.rej.done { border-color: var(--red); color: var(--red); }
.trash > button { width: 100%; border-radius: 10px; border: 1px solid var(--red); color: var(--red); background: #fff5f5; font-weight: 700; padding: 6px 0; }
</style>
...
for _, row in df.iterrows():
    ...
    for idx_s, (colname, label) in enumerate(STEPS):
        ...
        classes = "html-pill" + (" done" if done else "")
        ...
        grid[2 + idx_s].markdown(f"""
            <button class="{classes}" 
                    onclick="this.classList.toggle('done');
                             fetch('?toggle_request=' + JSON.stringify({{'id': {rid}, 'col': '{colname}'}}));">
                {pill_text}
            </button>
        """, unsafe_allow_html=True)
